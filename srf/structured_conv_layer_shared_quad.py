"""
Meta-parametrized SRF layer with sigma quadratically dependent on t.
"""
...
class Srf_layer_shared_quad(nn.Module):
    def __init__(self,
                inC,
                outC, 
                init_k,
                ...
                learn_sigma, 
                use_cuda):
        super(Srf_layer_shared_quad, self).__init__()
        ...
        if learn_sigma:
            self.scales_square = torch.nn.Parameter(torch.zeros([1],\
                            device=self.device), requires_grad=True)
            torch.nn.init.normal_(self.scales_square, mean=-0.0, std=2./3.)
    ...
    def forward(self, data, t):
        # Define sigma from the scale
        self.sigma = 2.0**(self.scales_square*t**2+self.scales*t+self.scale_bias)
